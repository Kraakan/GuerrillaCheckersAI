...
try:
    random_vs_hardcoded_2_checker_baseline = open("data/random_g_vs_hardcoded_2_checker_baseline.txt", "r")
except FileNotFoundError:
    print("Running random guerrilla vs. hardcoded COIN with 2 starting checker:")
    results = []
    lengths = []
    game = guerrilla_checkers.game(num_checkers=2)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    COIN = DQN.HardCoded(0, game, device)
    for i in range(num_games):
        game.reset()
    ...
    g_win_rate = results.count(-1)/precentage_denominator
    c_win_rate = results.count(1)/precentage_denominator
    with open("data/random_g_vs_hardcoded_2_checker_baseline.txt", "w") as f:
        f.write("Guerrilla wins: " + str(g_win_rate) + " %\n" + "COIN wins: " + str(c_win_rate) + " %\n")
        f.write("Avg. game length: " + str(avg_length))
        f.close()